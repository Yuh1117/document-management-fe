...
const featureItems = [
    { titleKey: "landing.features.document_management.title", descriptionKey: "landing.features.document_management.description", icon: FolderKanban },
    { titleKey: "landing.features.search.title", descriptionKey: "landing.features.search.description", icon: Search },
    { titleKey: "landing.features.sharing.title", descriptionKey: "landing.features.sharing.description", icon: Share2 },
    { titleKey: "landing.features.ai_summary.title", descriptionKey: "landing.features.ai_summary.description", icon: Sparkles },
];
...
const documentItems = [
    ...
    { nameKey: "landing.mock.documents.report", mimeType: "application/vnd.openxmlformats-officedocument.wordprocessingml.document" },
    { nameKey: "landing.mock.documents.plan", mimeType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" },
];
                    ...
                    <div className="flex flex-col justify-center">
                        ...
                        <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                            ...
                            <Button size="lg" variant="outline" asChild>
                                <Link to="/signup">{t("landing.cta.signup")}</Link>
                            </Button>
                        </div>
                                        ...
                                        {navigationItems.map((item) => {
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    className={`flex size-10 items-center justify-center rounded-md ${item.active
                                        ...
                                        <section>
                                            ...
                                            <div className="grid gap-4 sm:grid-cols-[repeat(2,minmax(0,200px))]">
                                                {folderItems.map((folderKey) => (
                                                    <div key={folderKey} className="flex h-12 items-center justify-between rounded-lg border bg-background px-4 shadow-xs transition-colors hover:bg-input/50">
                                        ...
                                        <section className="mt-6">
                                            ...
                                            <div className="grid gap-5 sm:grid-cols-3">
                                                {documentItems.map((document) => {
                                                    const { icon: Icon, color } = getIconComponentByMimeType(document.mimeType);
                                                    const iconClassName = getIconColorClassName(color);
                                                    const iconStyle = getIconColorStyle(color);

                                                    return (
                                                        <div key={document.nameKey} className="rounded-lg border bg-background p-4 shadow-xs transition-colors hover:bg-input/50">
                                                            <div className="mb-4 flex items-center justify-between">
                                                                <div className="flex min-w-0 items-center gap-2">
                                                                    <Icon
                                                                        ...
                                                                        style={iconStyle}
                                                                    />
                                                                    <span className="truncate text-xs font-medium">{t(document.nameKey)}</span>
                                                                </div>
            ...
            <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    {featureItems.map((item) => {
                        ...
                        return (
                            <article key={item.titleKey} className="rounded-lg border bg-card p-5 transition-colors hover:bg-input/50">
