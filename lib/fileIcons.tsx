...
export const getIconComponentByMimeType = (mimeType: string) => {
  ...
  switch (type) {
    case 'text':
      ...
      if (subtype === 'md' || subtype === 'markdown' || subtype === 'x-markdown')
        return { icon: FaMarkdown, color: '#3e8a9d' }
      break
    case 'image':
    ...
    case 'application':
      if (subtype === 'pdf') return { icon: FaRegFilePdf, color: '#e74c3c' }
      if (['zip', 'x-zip-compressed', 'vnd.rar', 'x-7z-compressed'].includes(subtype))
        return { icon: FaRegFileZipper, color: '#95a5a6' }
      if (
        ['msword', 'vnd.openxmlformats-officedocument.wordprocessingml.document'].includes(subtype)
      )
      ...
      if (
        ['powerpoint', 'vnd.openxmlformats-officedocument.presentationml.presentation'].includes(
          subtype
        )
      )
