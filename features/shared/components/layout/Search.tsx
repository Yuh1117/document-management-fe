...
const SearchBar = () => {
  ...
  const handleSearch = (value: string) => {
    ...
    nav.push(`/search?${query.toString()}`)
    setIsOpen(false)
  }
  ...
  const handleAdvancedSearch = () => {
    ...
    nav.push(`/advanced-search/?${query.toString()}`)
    setSearchValue('')
    setShowAdvanced(false)
  }

  const openAdvancedSearch = () => {
    if (searchValue.trim()) setKw(searchValue.trim())
  ...
  const filteredHistory = searchValue.trim()
    ? searchHistory.filter((item) => item.label.toLowerCase().includes(searchValue.toLowerCase()))
    : searchHistory
  ...
  return (
    <div className="flex w-full max-w-lg items-center gap-2">
      <div className="relative min-w-0 flex-1">
        <div
          className={cn(
            'flex items-center rounded-xl border bg-popover px-3 text-popover-foreground',
        ...
        {isOpen && (
          <div className="absolute top-full left-0 z-10 w-full rounded-b-xl border border-t-0 bg-popover shadow-lg">
            ...
            <div className="p-1">
              ...
              <HoverCard>
                <HoverCardTrigger>
                  <Button variant="ghost" className="rounded-xl" onClick={openAdvancedSearch}>
                    <SlidersHorizontal className="size-4 opacity-60" />
                    {t('search.advanced_label')}
                  </Button>
                </HoverCardTrigger>
                <HoverCardContent>{t('search.advanced_desc')}</HoverCardContent>
              </HoverCard>
            </div>
          </div>
        )}
