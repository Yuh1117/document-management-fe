  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg" aria-describedby={undefined}>
        ...
        <DialogDescription>{t('document.version_desc')}</DialogDescription>
            ...
            {documentVersions.length > 0 &&
              documentVersions.map((d) => (
                ...
                >
                  <div className="flex flex-col">
                    <div>
                      {t('document.version_number', { number: d.versionNumber })}{' '}
                      <span className="text-muted-foreground text-sm">{d.name}</span>
                    </div>
                    <div className="text-muted-foreground text-xs">{formatTime(d.createdAt)}</div>
                  </div>
