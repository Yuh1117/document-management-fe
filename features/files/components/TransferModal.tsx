...
import { isDocument, isFolder } from '@/lib/format'
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg" aria-describedby={undefined}>
        <DialogHeader>
          <DialogTitle>
            {mode == 'copy'
              ? t('transfer.copy_title', { name: data?.name })
              : t('transfer.move_title', { name: data?.name })}
          </DialogTitle>
        </DialogHeader>
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      selectedFolder === f.folder.id ? 'bg-accent' : '',
                      isInvalid ? 'text-muted-foreground cursor-not-allowed' : 'hover:bg-accent'
                    )}
                    onClick={() => {
