...
const ShareModal = ({ data, open, onOpenChange }: Props) => {
  ...
  const saveShare = async (
    data: IDocument | IFolder,
    people: { email: string; shareType: string }[]
  ): Promise<boolean> => {
    try {
      setSharing(true)
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <Avatar className="w-10 h-10">
                          <AvatarImage
                            src={(data?.createdBy?.avatar as string) || '/placeholder.svg'}
                            alt="avatar"
                          />
                      ...
                      <div className="bg-muted p-2 text-sm rounded-lg">{t('share.owner')}</div>
                    </div>
                          ...
                          <div>
                            <p
                              className={cn(
                                ...
                                person.remove && 'line-through text-muted-foreground'
                              )}
                            >
                              {person.name}
                            </p>
                        ...
                        {groups.map((g, index) => (
                          ...
                          >
                            ...
                            <Check
                              className={cn(
                                'ml-auto',
                                value === g.name ? 'opacity-100' : 'opacity-0'
                              )}
                            />
                          </CommandItem>
                        ))}
